...
With TYPO3 installed and root page 1 created, run `podman compose exec web vendor/bin/typo3 cache:flush`, `podman compose exec web vendor/bin/typo3 extension:setup --extension=agency_theme --no-interaction`, `podman compose exec web php config/seed-demo.php`, then flush the cache again. The script adds editable Home, Work, Contact and a hidden Components demo page without duplicating records on later runs. The demo is served at `http://localhost:8080/`; inspect Pricing, Video and Gallery at `http://localhost:8080/components`. The optional distribution package uses TYPO3 Initialisation instead of this development seed script.
